...
GeneRowNum = function(GENELIST) {
    ...
    START = as.numeric(GENELIST$START)
    END = as.numeric(GENELIST$END)
    NBUF = dim(GENELIST)[1]
    ROWINDX = rep(1, NBUF)
...
ReadSMRData = function(plotfile)
{
    ...
    if(keywords[1]!=key[1])
    {
        stop("ERROR: plot file is not correct!")
    }
...
SMRLocusPlot = function(data=SMRData, probeNEARBY=NULL,smr_thresh=NULL, smr_thresh_plot=NULL, heidi_thresh=NULL, plotWindow=NULL,pointsize=20,max_anno_probe=16,anno_selfdef=TRUE)
{
    ...
    if(length(smr_thresh)==0){
        stop("ERROR: please specify the threshold of SMR test!")
    }
    if(length(heidi_thresh)==0){
        stop("ERROR: please specify the threshold of HEIDI test!")
    }
    if(length(plotWindow)==0){
        stop("ERROR: please specify the plot window size!")
    }
    if(length(which(is.na(data$SMR[,3])))>0)
    {
        stop("ERROR: Some probes' physical positon is missing!")
    }
    idx=match(data$probeID,data$SMR[,1])
    if(length(idx)==0){
        stop("ERROR: Plot file is not generated correctly, can't find target probe!")
    }
    ...
    probePLOT=smrprobes
    probePLOT=unique(c(data$probeID,probePLOT,probeNEARBY))
    ...
    if(length(idx)>0){
        ...
        stop("ERROR: Plot file is not generated correctly, please report this bug!")
    }
    ...
    xmin = min(bp, na.rm=T) - 0.001
    xmax = max(bp, na.rm=T) +0.001
    ...
    for( k in 1 : nprobePLOT ) {
        ...
        eqtl.min = 0
        eqtl.max = ceiling(max(pvalbuf))
        eqtl.max =ceiling(eqtl.max *1.25)
    ...
    ybuf = -log10(as.numeric(smr_thresh))
    dev_anno = yMAX/9
...
SMREffectPlot = function(data=SMRData, trait_name="",cisWindow=2000, transWindow=5000, pointsize=20)
{
    ...
    if(length(snbuf)==0) {
        stop("ERROR: no eQTL infomation found for probe")
    }
    ...
    {
        stop("ERROR: no SMR reslult for probe")
    }
    ...
    bZX_tmp = bZX[cisIndx]
    seZX_tmp = seZX[cisIndx]
    zZX_tmp = bZX_tmp/seZX_tmp
    bZY_tmp = bZY[cisIndx]
    seZY_tmp = seZY[cisIndx]
    zZY_tmp = bZY_tmp/seZY_tmp
    ...
    if(ntrans>0) {
        ...
        bZX_tmp = bZX[transIndx]
        seZX_tmp = seZX[transIndx]
        zZX_tmp = bZX_tmp/seZX_tmp
        bZY_tmp = bZY[transIndx]
        seZY_tmp = seZY[transIndx]
        zZY_tmp = bZY_tmp/seZY_tmp
        par(new=TRUE)
        for( k in 1 : ntrans ) {
            colbuf = col_trans
    ...
    col_led = c(col_top, col_cis)
    pch_led = c(pch_top, pch_cis)
    if(ntrans>0) {
        lstr=c(lstr, "trans-eQTL")
        col_led = c(col_led, col_trans)
